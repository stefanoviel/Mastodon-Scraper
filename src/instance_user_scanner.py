...
class InstanceScanner:
    ...
    async def add_id_following_follower_queue(self, user_id, user_url):
        ...
        await self.following_queue.put(
            (
                "https://{}/api/v1/accounts/{}/following/".format(
                    self.instance_name, user_id
                ),
                user_url,
            )
        )
        await self.follower_queue.put(
            (
                "https://{}/api/v1/accounts/{}/followers/".format(
                    self.instance_name, user_id
                ),
                user_url,
            )
    ...
    async def get_following_followers(
        ...
        try:
            async with session.get(url, params=params, timeout=5) as response:
                ...
                if (
                    len(res) > 0 and "error" not in res
                ):  # check there is no too many error
    ...
    async def request_ids(
        ...
        for completed_requests in range(n_request):
            if completed_requests < 250 and not self.id_queue.empty():
                ...
                tasks.append(
                    asyncio.create_task(self.get_id_from_url(session, user_url))
                )
            else:
                break
    ...
    async def request_follower_following(
        self, tasks: list, session: aiohttp.ClientSession, n_request: int
    ) -> None:
        ...
        while n_request > 0:
            ...
            # print(n_request )
            completed_follower, n_request = await self.get_from_queue(
                self.follower_queue, tasks, session, n_request
            )
            completed_following, n_request = await self.get_from_queue(
                self.following_queue, tasks, session, n_request
            )
    ...
    async def request_id_and_url(self, n_request, session):
        ...
        n_request = await self.request_ids(
            tasks, session, n_request
        )  # request numerical id of users we only know the name
